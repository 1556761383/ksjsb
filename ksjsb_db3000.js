 ...
 * ========= 青龙--配置文件 =========
 * 变量格式: export ksjsbCookie='xxxxx'  多个账号用 @分割 或者 换行分割
 *
...
async function action_result() {
	...
	let result = await httpGet(url, `资产查询`);
...
async function user_info() {
	...
	let result = await httpGet(url, `用户查询`);
	...
	if (result.result == 1) {
		...
		msg += `用户信息: 失败 ❌ 了呢,原因未知！  ${JSON.parse(result)} \n `;
...
async function sign_info() {
	...
	let result = await httpGet(url, `签到查询`);

	if (result.data.nebulaSignInPopup.todaySigned == false) {
		console.log(`签到结果: 今天未签到,去签到喽!\n`);
		msg += `签到结果: 今天未签到,去签到喽!\n`;
		await $.wait(3 * 1000);
		await signin();
	} else if (result.data.nebulaSignInPopup.todaySigned == true) {
		console.log(`签到结果: 今天已签到,明天再来吧!\n`);
		msg += `签到结果: 今天已签到,明天再来吧!\n`;
	} else {
		console.log(`签到结果: 失败 ❌ 了呢,原因未知！  ${result} \n`);
		msg += `签到结果: 失败 ❌ 了呢,原因未知！  ${JSON.parse(result)} \n `;
	}
}
...
async function signin() {
	...
	if (result.result == 1) {
		console.log(`签到: ${result.data.toast} ,获得 ${result.data.totalCoin} 金币 \n`);
		msg += `签到: ${result.data.toast} ,获得 ${result.data.totalCoin} 金币 \n`;
	} else if (result.result == 10901) {
		console.log(`签到: ${result.error_msg} \n`);
		msg += `签到: ${result.error_msg} \n`;
	} else {
		console.log(`签到: 失败 ❌ 了呢,原因未知！  ${result} \n`);
		msg += `签到: 失败 ❌ 了呢,原因未知！  ${JSON.parse(result)} \n `;
	}
}
...
async function box_info() {
	...
	let result = await httpGet(url, `查询宝箱动态`);

	if (result.result == 1) {
		if (result.data.openTime == -1) {
			console.log(`宝箱动态: 今日宝箱已开完,明天再来吧! \n`);
			msg += `宝箱动态: 今日宝箱已开完,明天再来吧! \n`;
		} else if (result.data.openTime != 0) {
			console.log(`宝箱动态: 冷却中, 剩余时间: ${result.data.openTime / 1000 / 60}分钟\n`);
			msg += `宝箱动态: 冷却中, 剩余时间: ${result.data.openTime / 1000 / 60}分钟\n`;
		} else {
			console.log(`宝箱动态: 冷却完成，准备开宝箱! \n`);
			msg += `宝箱动态: 冷却完成，准备开宝箱! \n`;
			await $.wait(3 * 1000);
			await open_box();
		}
		...
		console.log(`宝箱动态: ${result.error_msg} \n`);
		msg += `宝箱动态: ${result.error_msg} \n`;
	} else {
		console.log(`宝箱动态: 失败 ❌ 了,原因未知！  ${result} \n`);
		msg += `宝箱动态: 失败 ❌ 了,原因未知！  ${JSON.parse(result)} \n `;
	}
}
...
async function open_box() {
	...
	if (result.result == 1) {
		console.log(`开箱结果: 获得 ${result.data.commonAwardPopup.awardAmount} 金币!\n`);
		msg += `开箱结果: 获得 ${result.data.commonAwardPopup.awardAmount} 金币!\n`;

	} else {
		console.log(`开箱结果: 失败 ❌ 了呢,原因未知！  ${result} \n`);
		msg += `开箱结果: 失败 ❌ 了呢,原因未知！  ${JSON.parse(result)} \n `;
	}
}
...
async function Share(id) {
	...
	let result = await httpGet(url, `分享3000`);

	if (result.result == 1) {
		console.log(`分享3000: 获得 ${result.data.amount} 金币!\n`);
		msg += `分享3000: 获得 ${result.data.amount} 金币!\n`;
	} else if (result.result == 14004) {
		console.log(`分享3000: 今天已分享,明天再来吧!\n`);
		msg += `分享3000: 今天已分享,明天再来吧!\n\n`;
	} else {
		console.log(`分享3000: 失败 ❌ 了,原因未知！  ${result} \n`);
		msg += `分享3000: 失败 ❌ 了,原因未知！  ${JSON.parse(result)} \n `;
	}
}
	...
	return new Promise((resolve) => {
		...
		$.get(url, async (err, resp, data) => {
			try {
				...
				console.log(`\n 【网抑云】: ${data.content}  by--${data.music}`);
